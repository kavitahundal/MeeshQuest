...
public class OldAvlGTree<K, V> extends AbstractMap<K, V> implements SortedMap<K, V> {
	...
	public OldAvlGTree(final Comparator<? super K> comp) {
		this(comp, 1);
	}

	public OldAvlGTree(final int g) {
		this(null, 1);
	}
	...
	public OldAvlGTree(java.util.Map<? extends K, ? extends V> m) {
		this();
		this.putAll(m);
	}

	public OldAvlGTree(java.util.SortedMap<? extends K, ? extends V> m) {
		this();
		this.putAll(m);
	}
...
//			parent.right = null;
	...
	class MySet extends AbstractSet<java.util.Map.Entry<K, V>> implements Set<java.util.Map.Entry<K, V>> {
		...
		@Override
		public int size() {
			return OldAvlGTree.this.size();
		}

		@Override
		public boolean isEmpty() {
			return OldAvlGTree.this.isEmpty();
		}

		@Override
		public boolean contains(Object o) {
			@SuppressWarnings("unchecked")
			AvlNode<K, V> node = OldAvlGTree.this.find(((java.util.Map.Entry<K, V>) o).getKey());
		...
		public Iterator<java.util.Map.Entry<K, V>> iterator() {
			return new Iterator<java.util.Map.Entry<K, V>>() {
...

				{
					...
					this.fillList(entryList, OldAvlGTree.this.root);
...
//					System.out.println("I just added " + node.key); // TODO
				...
				public void remove() {
					this.wrapper.remove();
					OldAvlGTree.this.remove(this.current.getKey());
				}
		...
		public boolean add(java.util.Map.Entry<K, V> e) {
			return !e.getValue().equals(OldAvlGTree.this.put(e.getKey(), e.getValue()));
		}

		@Override
		public boolean remove(Object o) {
			return OldAvlGTree.this.remove(o) != null;
		}
...
//			Iterator<java.util.Map.Entry<K, V>> iter = this.entrySet().iterator();//TODO fix
			Iterator<java.util.Map.Entry<K, V>> iter = OldAvlGTree.this.entrySet().iterator();
			while (iter.hasNext()) {
		...
		private boolean outOfBounds(K key) {
			return OldAvlGTree.this.comp.compare(key, this.low) < 0 || OldAvlGTree.this.comp.compare(key, this.high) >= 0;
		}
		...
		public boolean containsKey(Object key) {
			...
			return OldAvlGTree.this.containsKey(key);
		}

		@SuppressWarnings("unchecked")
		@Override
		public boolean containsValue(Object value) {
			AvlNode<K, V> root = this.getValidRoot(OldAvlGTree.this.root);
		...
		public V get(Object key) {
			...
			return OldAvlGTree.this.get(key);
		}
		...
		public V put(K key, V value) {
			...
			return OldAvlGTree.this.put(key, value);
		}
...
//				throw new IllegalArgumentException();
//			}
			return OldAvlGTree.this.remove(key);
		}

		@Override
		public int size() {
			return sizeAux(OldAvlGTree.this.root);
		}
		...
		private int sizeAux(AvlNode<K, V> node) {
			...
			if (OldAvlGTree.this.comp.compare(node.key, this.low) > 0) {
				size += this.sizeAux(node.left);
			}
			if (OldAvlGTree.this.comp.compare(node.key, this.high) < 0) {
		...
		private AvlNode<K, V> getValidRoot(AvlNode<K, V> node) {
			if (node == null) {
				...
				return node;
			} else if (OldAvlGTree.this.comp.compare(node.key, this.low) < 0) {
...
//			return node.right == null || this.outOfBounds(node.right.key) ? node.key : this.firstKeyAux(node.right);
			if (node == null) {
				return null;
			} else if (OldAvlGTree.this.comp.compare(node.key, this.high) >= 0) {
				// if too high go left
		...
		public SortedMap<K, V> subMap(K arg0, K arg1) {
			...
			return OldAvlGTree.this.subMap(arg0, arg1);
		}
